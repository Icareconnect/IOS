...
struct CollectionSizeProvider {
    ...
    func getHeightOfTableViewCell(for numberOfItems: Int, gridCount: Int) -> CGFloat {
        let numberOfGridRows = round((CGFloat(numberOfItems) / CGFloat(gridCount)) + 0.1)
        return (numberOfGridRows * cellSize.height) + (lineSpacing * (numberOfGridRows - 1.0)) + edgeInsets.top + edgeInsets.bottom
    }
}
